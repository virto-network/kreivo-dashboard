...
import(
  "https://cdn.jsdelivr.net/npm/virto-components@0.1.11/dist/virto-components.min.js"
)

import SDK from "https://cdn.jsdelivr.net/npm/@virtonetwork/sdk@0.0.4-alpha.39/dist/web/index.js"
  ...
  * {
  ...
  wa-dialog::part(base) {
    ...
    background:
      linear-gradient(
        180deg,
        rgba(255, 255, 255, 0.745) 0%,
        rgba(255, 255, 255, 0.634) 100%
      ),
      radial-gradient(
        84.04% 109.28% at 10.3% 12.14%,
        color-mix(in srgb, var(--green) 64.6%, transparent) 0%,
        color-mix(in srgb, var(--lightgreen) 44.9%, transparent) 98.5%
      );
    border-radius: 12px;
    box-shadow:
      0px 2px var(--Blurblur-3, 3px) -1px rgba(26, 26, 26, 0.08),
...
const registerFormTemplate = html`
  <form id="register-form">
    ...
    <div class="disclaimer">
      ...
      <strong>Channel Requirement:</strong> You must be a member of the Kreivo
      channel (<a
        href="https://matrix.to/#/#kreivo:virto.community"
        target="_blank"
        rel="noopener noreferrer"
        >#kreivo:virto.community</a
      >) on Element/Matrix to use this service. Please join the channel before
      proceeding.
    </div>
    <!-- Sign Up Link -->
    <div class="link">
...
export class VirtoConnect extends HTMLElement {
  ...
  initSDK() {
    ...
    try {
      ...
      console.log(
        `Virto SDK initialized with server: ${this.serverUrl} and provider: ${this.providerUrl}`,
      )
    ...
    switch (this.currentFormType) {
      case "register":
        const nameInput = this.shadowRoot.querySelector(
          'virto-input[name="name"]',
        )
        const registerUsernameInput = this.shadowRoot.querySelector(
  ...
  showFaucetConfirmation(username) {
    ...
    this.dispatchEvent(
      new CustomEvent("faucet-iframe-ready", {
        bubbles: true,
        detail: {
          username,
  ...
  completeFaucetFlow(username, faucetAccepted, faucetResult = null) {
    ...
    if (faucetAccepted && faucetResult) {
      successMsg.textContent =
        "Success! Your welcome bonus has been added to your account. You can now sign in."
    } else {
    ...
    this.dispatchEvent(
      new CustomEvent("register-complete", {
        bubbles: true,
        detail: {
          username,
          faucetAccepted,
  ...
  updateButtons() {
    ...
    if (this.currentFormType === "register") {
      ...
      actionButton.addEventListener(
        "click",
        async () => await this.submitFormRegister(),
      )
    } else {
      actionButton.setAttribute("label", "Sign In")
      actionButton.addEventListener(
        "click",
        async () => await this.submitFormLogin(),
      )
    }
  ...
  async submitFormRegister() {
    ...
    try {
      ...
      console.warn(
        "Sentry not available for transaction tracking:",
        sentryError,
      )
    }
    ...
    try {
      ...
      if (!name || name.trim() === "") {
        ...
        if (window.Sentry)
          Sentry.captureMessage(
            "Registration validation failed - missing name",
            "warning",
          )
        if (transaction) {
      ...
      if (!username || username.trim() === "") {
        ...
        if (window.Sentry)
          Sentry.captureMessage(
            "Registration validation failed - missing username",
            "warning",
          )
        if (transaction) {
          transaction.setStatus("invalid_argument")
      ...
      const registrationCheckSpan = transaction
      ...
      try {
        ...
        if (isRegistered) {
          ...
          if (window.Sentry) {
            Sentry.captureMessage(
              "Registration attempted for existing user",
              "info",
              {
                extra: { username },
              },
          ...
          if (errorMsg) {
            errorMsg.textContent =
              "This user is already registered. Please sign in instead."
            errorMsg.style.display = "block"
          }

          this.dispatchEvent(
            new CustomEvent("register-error", {
              bubbles: true,
              detail: {
                error:
                  "This user is already registered. Please sign in instead.",
      ...
      // Actual registration
      const registrationSpan = transaction
        ? transaction.startChild({
            op: "auth.register",
  ...
  async submitFormLogin() {
    ...
    try {
      ...
      if (!username || username.trim() === "") {
        ...
        if (window.Sentry)
          Sentry.captureMessage(
            "Login validation failed - missing username",
            "warning",
          )
        if (transaction) {
          transaction.setStatus("invalid_argument")
      ...
      if (!this.sdk || !this.sdk.auth) {
        ...
        errorMsg.textContent =
          "Please enable Demo Mode to initialize the connection."
