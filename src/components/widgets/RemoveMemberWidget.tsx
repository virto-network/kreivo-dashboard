...
export const RemoveMemberWidget: React.FC<WidgetProps> = ({
  ...
  const handleSubmitTransaction = async (
    ...
    try {
          ...
          try {
            ...
            if (itemData && itemData.owner) {
              ...
              try {
                ...
                const areEqual =
                  ownerBytes.length === userBytes.length &&
                  ownerBytes.every((byte, index) => byte === userBytes[index])

                if (areEqual) {
                  membershipId = itemId
                  console.log(
                    `Found membership ID ${itemId} for user ${resolvedAddress} in community ${communityIdNum}`,
                  )
                  break
                }
              } catch (decodeError) {
                console.warn(
                  `Could not decode addresses for comparison at item ${itemId}:`,
                  decodeError,
                )
                if (owner === resolvedAddress) {
                  membershipId = itemId
                  console.log(
                    `Found membership ID ${itemId} (direct match) for user ${resolvedAddress} in community ${communityIdNum}`,
                  )
                  break
                }
              }
      ...
      if (membershipId === null) {
        throw new Error(
          `User ${resolvedAddress} doesn't have a membership in community ${communityName}`,
        )
      }
  ...
  const handleCommunitySelect = (community: (typeof communities)[0]) => {
    ...
    if (parsed.hashtagStart !== null) {
      ...
      setTimeout(() => {
        const newPos =
          parsed.hashtagStart! + community.name.length + autoText.length + 1
        inputRef.current?.setSelectionRange(newPos, newPos)
        inputRef.current?.focus()
      }, 0)
    }
    ...
    <div className="widget-form" ref={containerRef}>
      ...
      <div className="widget-input-wrapper">
        ...
        <button
          ...
          onClick={() => {
            const parsed = parseInput(input, cursorPosition)
            const community = parsed.segments.find(
              (s) => s.type === "community",
            )
            if (community && parsed.userAddress) {
              handleSubmitTransaction(community.value, parsed.userAddress)
