  ...
  const handleAmountSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!sdk.auth?.sessionSigner) {
        throw new Error(
          "Session signer not available. Please try reconnecting your wallet.",
        )
      }

      console.log("Signing and submitting transaction with VirtoConnect...")

      const result = await sdk.custom.submitCallAsync(sdk.auth.sessionSigner, {
              ...
              {showAssetList && filteredAssets.length > 0 && (
                <div className="inline-select-dropdown">
                  {filteredAssets.map((asset, index) => (
                    ...
                    >
                      <span className="inline-select-prefix">
                        {asset.symbol}
                      </span>
                      {asset.name}
                    </div>
