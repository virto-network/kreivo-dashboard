...
export const CommandSearch: React.FC<CommandSearchProps> = ({
  ...
  const handleFocus = useCallback(() => {
    ...
    setTimeout(() => {
      const promptSection =
        containerRef.current?.querySelector(".prompt-section")
      if (promptSection) {
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (
        extrinsicMenuRef.current &&
        !extrinsicMenuRef.current.contains(event.target as Node)
      ) {
  ...
  useEffect(() => {
    if (externalTrigger) {
      const command = {
        ...
        label:
          externalTrigger.commandId === "send-transaction"
            ? "Send Transaction"
            : externalTrigger.commandId,
        keywords: [],
        related: [],
  ...
  const handleSendCustomExtrinsic = useCallback(
    async (encodedCallHex: string) => {
      if (!sdk) {
        showErrorNotification(
          "Connection Error",
          "Please connect to Virto first",
        )
        return
      }
  ...
  return (
    <>
      <div className="command-search-container" ref={containerRef}>
        <div
          className={`prompt-section ${isFocused ? "prompt-section-focused" : ""}`}
        >
            ...
            <>
              ...
              {results.length > 0 && (
                <div className="prompt-buttons">
                  ...
                  <div
                    className="command-search-extrinsic-menu-container"
                    ref={extrinsicMenuRef}
                  >
                    <button
              ...
              {showExtrinsicEditor && (
                <div className="command-search-extrinsic-editor">
                  <div className="command-search-extrinsic-editor-header">
                    <h3 className="command-search-extrinsic-editor-title">
                      Create Custom Extrinsic
                    </h3>
                    <button
