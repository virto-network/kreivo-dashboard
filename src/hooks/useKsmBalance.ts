...
const getKsmPrice = async (): Promise<number> => {
  try {
    const response = await fetch(
      "https://sapi.coincarp.com/api/v1/his/coin/trend?code=kusama&type=s",
    )
  ...
  useEffect(() => {
    ...
    const fetchBalance = async () => {
      try {
        ...
        const accountData = await typedApi.query.System.Account.getValue(
          accountAddress,
        ).catch((err: any) => {
        ...
        try {
          const assetId = {
            ...
            value: { id: 1000, pallet: 50, index: 50000002 },
          }
          console.log(
            "Querying DUSD balance with assetId:",
            assetId,
            "address:",
            accountAddress,
          )

          const dusdAccountData = await typedApi.query.Assets.Account.getValue(
            assetId,
            accountAddress,
          ).catch((err: any) => {
          ...
          if (dusdAccountData) {
            ...
            console.log(
              "DUSD balance from storage:",
              dusdAmount,
              "raw:",
              dusdBalance.toString(),
            )
          } else {
