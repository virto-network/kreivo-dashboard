...
export const useTransactionListener = () => {
  const { addTransaction, updateTransactionStatus, updateTransactionHash } =
    useTransactionStore()

  const extractTransactionData = useCallback(
    (event: CustomEvent): ExtractedTransactionData | null => {
      try {
  ...
  const handleStatusUpdate = useCallback(
    (event: CustomEvent, status: "included" | "finalized" | "failed") => {
      const transactionId = validateTransactionId(event)
      const transactionData = extractTransactionData(event)

      if (!transactionId) {
        console.warn(
          `Cannot update transaction status to ${status}: missing transaction ID`,
        )
        return
      }
  ...
  const eventHandlers = useCallback(
    () => ({
      ...
      included: () => (event: CustomEvent) =>
        handleStatusUpdate(event, "included"),
      finalized: () => (event: CustomEvent) =>
        handleStatusUpdate(event, "finalized"),
