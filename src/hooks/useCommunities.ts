...
const LOG_ENABLED =
  localStorage.getItem("communities-logs") === "true" || import.meta.env.DEV
...
if (import.meta.env.DEV) {
  ;(window as any).enableCommunitiesLogs = () =>
    localStorage.setItem("communities-logs", "true")
  ;(window as any).disableCommunitiesLogs = () =>
    localStorage.removeItem("communities-logs")
}
    ...
    const fetchCommunities = async () => {
      try {
        ...
        try {
          ...
          try {
            const tracksIdsResult = await (
              api.query as any
            ).CommunityTracks?.TracksIds?.getValue().catch((err: any) => {
            ...
            throw new Error(
              "Could not query CommunityTracks.TracksIds: " +
          ...
          for (const communityId of communityIds) {
            try {
              ...
              const tracksResult = await (
                api.query as any
              ).CommunityTracks?.Tracks?.getValue(communityId)
              log.debug(`Community ${communityId} tracksResult:`, tracksResult)

              const itemResult = await (
                api.query as any
              ).CommunityMemberships?.Item?.getEntries(communityId)
              ...
              try {
                ...
                log.debug(
                  `Community ${communityId} itemResult (ValueSet):`,
                  itemResult,
                )
                ...
                log.debug(
                  `Community ${communityId}: members=${members} (from ValueSet length)`,
                )
              } catch (itemErr) {
                log.warn(
                  `Community ${communityId}: error querying item/members:`,
                  itemErr,
                )
                members = 0
              }
              ...
              if (tracksResult) {
                ...
                if (track.name) {
                  if (typeof track.name.asText === "function") {
                    try {
                      ...
                      log.warn(
                        `Community ${communityId}: error extracting name from FixedSizeBinary:`,
                        err,
                      )
                      ...
                      try {
                        ...
                        log.warn(
                          `Community ${communityId}: error extracting name from bytes:`,
                          bytesErr,
                        )
                      }
                    }
                  } else if (Array.isArray(track.name)) {
                    const filteredName = track.name.filter(
                      (b: number) => b !== 0,
                    )
                    name = String.fromCharCode(...filteredName)
                ...
                log.warn(
                  `Community ${communityId}: no tracks result, using default name`,
                )
              }
              ...
              log.debug(
                `Community ${communityId}: name="${name}", members=${members}`,
              )
            } catch (err) {
              log.error(
                `Error querying data for community ${communityId}:`,
                err,
              )
          ...
          log.info(
            `Successfully fetched ${communitiesList.length} communities:`,
