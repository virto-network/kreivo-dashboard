...
const LOG_ENABLED = localStorage.getItem('initiatives-logs') === 'true' || import.meta.env.DEV;
...
if (import.meta.env.DEV) {
  (window as any).enableInitiativesLogs = () => localStorage.setItem('initiatives-logs', 'true');
  (window as any).disableInitiativesLogs = () => localStorage.removeItem('initiatives-logs');
}
    ...
    const fetchInitiatives = async () => {
      try {
        ...
        try {
          ...
          const trackQueueResult = await (api.query as any).CommunityReferenda?.TrackQueue?.getValue(communityId).catch((err: any) => {
            log.error('Error querying TrackQueue:', err);
            return null;
          });
          
          log.debug(`trackQueue result for community ${communityId}:`, trackQueueResult);
          ...
          log.info(`Found ${trackInfos.length} initiatives in trackQueue:`, trackInfos);
          ...
          const trackInfosToProcess = limit ? sortedTrackInfos.slice(0, limit) : sortedTrackInfos;
          ...
          for (const trackInfo of trackInfosToProcess) {
            try {
              ...
              const referendumInfo = await (api.query as any).CommunityReferenda?.ReferendumInfoFor?.getValue(trackInfo.id).catch(() => null);
              
              if (referendumInfo) {
                log.debug(`Referendum ${trackInfo.id} info:`, referendumInfo);
                

                const ongoing = referendumInfo.Ongoing || referendumInfo.ongoing || referendumInfo;
                ...
                try {
                  const metadata = await (api.query as any).CommunityReferenda?.MetadataOf?.getValue(trackInfo.id).catch(() => null);
                  if (metadata) {

                    if (Array.isArray(metadata) || metadata instanceof Uint8Array) {
                      const bytes = Array.from(metadata);
                      const filteredBytes = bytes.filter((b: number) => b !== 0);
                      if (filteredBytes.length > 0) {
                        const decodedName = String.fromCharCode(...filteredBytes);
                        if (decodedName.trim()) {
                  ...
                  log.debug(`Could not get metadata for referendum ${trackInfo.id}:`, metadataErr);
                }
                ...
                log.debug(`Initiative ${trackInfo.id}: name="${name}", progress=${progress}%, ayes=${ayes}, nays=${nays}`);
              } else {

                log.warn(`Could not get details for referendum ${trackInfo.id}, using basic info`);
                initiativesList.push({
                  id: trackInfo.id,
              ...
              log.error(`Error fetching details for referendum ${trackInfo.id}:`, err);
          ...
          log.info(`Successfully fetched ${initiativesList.length} initiatives:`, initiativesList.map(i => ({ id: i.id, name: i.name, progress: i.progress })));
