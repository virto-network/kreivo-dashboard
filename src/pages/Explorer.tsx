        ...
        try {
          ...
          const fetchBlockData = async (
            ...
            try {
              ...
              const body = await firstValueFrom(chainHead.body$(hash)).catch(
                (err) => {
              ...
              if (blockNumber === null) {
                console.log(
                  "Could not extract block number from header:",
                  header,
                )
                return null
              }

              console.log("Block data created:", {
                blockNumber,
                hash,
                extrinsicsCount,
              })
            ...
            next: async (event) => {
              try {
                ...
                if (event.type === "newBlock") {
                  const blockHash =
                    (event as any).blockHash || (event as any).hash
                ...
                if (event.type === "initialized") {
                  ...
                  if (finalizedHash && typeof finalizedHash === "string") {
                    console.log(
                      "Fetching initialized block data for hash:",
                      finalizedHash,
                    )
                ...
                if (event.type === "finalized") {
                  const finalizedHash =
                    (event as any).hash || (event as any).blockHash
          ...
          firstValueFrom(chainHead.finalized$.pipe(take(1)))
            .then(async (finalizedHash) => {
              ...
              if (finalizedHash && typeof finalizedHash === "string") {
                ...
                if (blockData) {
                  ...
                  for (let i = 0; i < 4; i++) {
                    try {
                      const currentHeader = await firstValueFrom(
                        chainHead.header$(currentHash),
                      ).catch(() => null)
                      if (
                        currentHeader &&
                        "parentHash" in currentHeader &&
                        currentHeader.parentHash
                      ) {
                        const parentHash: string =
                          typeof currentHeader.parentHash === "string"
                            ? currentHeader.parentHash
              ...
              <div key={block.hash || index} className="block-card">
                ...
                <div className="block-body">
                  <div className="block-info-row">
                    ...
                    <span className="block-value block-hash">
